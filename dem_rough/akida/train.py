...
model_keras = keras.models.Sequential([
    keras.layers.Conv2D(
        filters=8, kernel_size=3, input_shape=(INPUT_HEIGHT, INPUT_WIDTH, 1), strides=1, ),
    keras.layers.BatchNormalization(),
    keras.layers.ReLU(),
...
model_keras.fit(x_train, y_train, epochs=100, validation_split=0.1, batch_size=12)
...
print('Test accuracy:', score[1])
# %%
out = model_keras.predict(x_test[0:1])
change_output(out[0])
# %%
