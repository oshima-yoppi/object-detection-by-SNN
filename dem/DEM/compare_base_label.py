...
target_DEM_path = (
    r"G:\マイドライブ\DEM\64pix_dem(lidar_noisy)_boulder_evaluate\model(t-100)"  # 1/26
)
print("START")
...
def iou_score(outputs, labels):
    ...
    intersection = (np.uint64(output) & np.uint64(label)).sum(
        (0, 1)
    )  # will be zero if Trueth=0 or Prediction=0
    union = (np.uint64(output) | np.uint64(label)).sum(
        (0, 1)
