...
def main():
    ...
    def analysis_segmentation(pred, label):
        ...
        pred_danger = (
            pred_danger.reshape((INPUT_HEIGHT, INPUT_WIDTH))
            .to("cpu")
            .detach()
            .numpy()
            .copy()
        ...
        label_np = (
            label.reshape((INPUT_HEIGHT, INPUT_WIDTH)).to("cpu").detach().numpy().copy()
        )
        # print(pred.shape)
        # print(label.shape)
        # exit()
    ...
    def save_img(number, events, pred_pro, label, results, pdf_output):
        ...
        # 1 表作成
        data = np.array([[tp, tn], [fp, fn], [precision_, recall_],])
